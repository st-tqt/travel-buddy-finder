  ...
  return (
    <div className={`flex flex-col ${isMine ? 'items-end' : 'items-start'} mb-4`}>
      <div className="text-xs text-gray-500 mb-1 mx-1">
        {isMine ? 'Bạn' : message.senderName || message.senderEmail || message.userEmail || message.senderId} • {new Date(message.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
      </div>
